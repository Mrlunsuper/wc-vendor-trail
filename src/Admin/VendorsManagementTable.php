...
class VendorsManagementTable extends \WP_List_Table {
	...
	private function get_vendor_roles() {
		return apply_filters('vendor_roles', ['vendor', 'pending_vendor']);
	}
 ...
 */
	private function records_count() {
		$roles = $this::get_vendor_roles();
		$users = get_users(['role__in' => $roles, 'count_total' => true]);
		return count($users);
	}
 ...
 */
	...
	public function column_cb($item) {
		return sprintf(
			'<input type="checkbox" name="%s[]" value="%s" />', $this->_args['singular'], $item['ID']
		);
	}
